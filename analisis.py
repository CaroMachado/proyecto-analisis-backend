...
# --- CAMBIO CLAVE: Importar las librerías para ejecutar el modelo localmente ---
from transformers import pipeline
import torch

# --- CONFIGURACIÓN DE IA LOCAL ---
# CAMBIO CLAVE: Cargar el modelo de IA UNA SOLA VEZ cuando la aplicación se inicia.
# Esto puede tardar varios minutos en el primer arranque de Render mientras descarga el modelo.
print("INFO: Cargando el modelo de análisis de sentimiento. Esto puede tardar...")
try:
    sentiment_pipeline = pipeline(
        "sentiment-analysis",
        model="pysentimiento/robertuito-sentiment-analysis",
        device=-1  # Forzar el uso de CPU (-1) en lugar de GPU (0)
    )
...
def analizar_sentimiento_con_ia(texto):
    ...
    Si el modelo no se cargó, devuelve None para que se use el método de respaldo.
    """
...
def generar_grafico_valoraciones_diarias(df):
    ...
    ax1.legend(title='Calificación (IA)')
    ...
    for i, txt in enumerate(daily_satisfaction.values):
        ax2.annotate(txt, (i, daily_satisfaction.values[i]), textcoords="offset points", xytext=(0,10), ha='center', color='purple', weight='bold')
    ax2.set_ylabel('Índice de Satisfacción (-100 a 100)', fontsize=12, color='purple')
    ax2.tick_params(axis='y', labelcolor='purple')
    ax2.set_ylim(min(-100, daily_satisfaction.min() - 10 if not daily_satisfaction.empty else 0), max(100, daily_satisfaction.max() + 10 if not daily_satisfaction.empty else 100))
    fig.tight_layout()
    buf = io.BytesIO()
...
def procesar_datos(archivo_excel):
    ...
    columnas_necesarias = ['fecha', 'hora', 'sala', 'sector', 'comentarios', 'calificacion_descripcion', 'puntos_criticos', 'destacados']
    ...
    def asignar_grupo(row):
        ...
        if 'atención' in sector_lower: return 'Atención al Cliente'
    ...
    resumen_grupos = []
    analisis_detallado = []
    orden_grupos = ['Baños', 'Atención al Cliente', 'Cajas', 'Restaurantes', 'Autoservicios', 'Traslados', 'VIP']
    todas_oportunidades = []
    todos_destacados = []
        ...
        if not df_grupo.empty:
            ...
            for sector_especifico, df_sector_esp in df_grupo.groupby('sector_original'):
                ...
                if analisis:
                    ...
                    todas_oportunidades.extend(analisis['oportunidades_mejora'])
                    todos_destacados.extend(analisis['puntos_destacados'])
    ...
    resumen_ia = generar_resumen_ia_simple(todas_oportunidades, todos_destacados)
            
    return {
        "informe_periodo": periodo,
        "analisis_general": analisis_general,
        "resumen_por_grupos": resumen_grupos,
