...
app.post('/procesar', upload.single('archivoExcel'), async (req, res) => {
    try {
        ...
        worksheet.eachRow((row, rowNumber) => { if (rowNumber === 1) return; try { const jsDate = parseDateTime(row.getCell(columnMap['fecha']).value, row.getCell(columnMap['hora']).value); if (!jsDate) return; const diaSemana = DIAS_SEMANA[jsDate.getUTCDay()]; const fechaStr = jsDate.toLocaleDateString('es-AR', { day: '2-digit', month: '2-digit', year: 'numeric', timeZone: 'UTC' }); const hora = jsDate.getUTCHours(); const sector = String(row.getCell(columnMap['sector']).value || '').trim(); const ubicacion = String(row.getCell(columnMap['ubicacion']).value || '').trim(); const sectorKey = sector && ubicacion ? `${sector} - ${ubicacion}` : (sector || ubicacion); if (!sectorKey) return; const calificacionDesc = String(row.getCell(columnMap['calificacion_descripcion']).value || '').trim(); const comentario = String(row.getCell(columnMap['comentarios'])?.value || '').trim(); const puntoCritico = String(row.getCell(columnMap['puntos_criticos'])?.value || '').trim(); const puntoDestacado = String(row.getCell(columnMap['destacados'])?.value || '').trim(); if (!processedData.porDia[diaSemana]) { processedData.porDia[diaSemana] = { muy_positivas: 0, positivas: 0, negativas: 0, muy_negativas: 0, total: 0, sectoresDelDia: [] }; dailyDetails[diaSemana] = { valoracionesPorHora: Array.from({ length: 24 }, () => ({ muy_positivas: 0, sectoresPositivos: {} })), sectores: {} }; } if (!processedData.fechas.includes(fechaStr)) { processedData.fechas.push(fechaStr); } if (!processedData.porSector[sectorKey]) { processedData.porSector[sectorKey] = { muy_positivas: 0, positivas: 0, negativas: 0, muy_negativas: 0, total: 0 }; } if (!dailyDetails[diaSemana].sectores[sectorKey]) { dailyDetails[diaSemana].sectores[sectorKey] = { muy_positivas: 0, positivas: 0, negativas: 0, muy_negativas: 0, total: 0, criticos: {}, destacados: {}, comentarios: [] }; } processedData.general.total++; processedData.porDia[diaSemana].total++; processedData.porHora[hora].total++; processedData.porSector[sectorKey].total++; dailyDetails[diaSemana].sectores[sectorKey].total++; if (puntoCritico) dailyDetails[diaSemana].sectores[sectorKey].criticos[puntoCritico] = (dailyDetails[diaSemana].sectores[sectorKey].criticos[puntoCritico] || 0) + 1; if (puntoDestacado) dailyDetails[diaSemana].sectores[sectorKey].destacados[puntoDestacado] = (dailyDetails[diaSemana].sectores[sectorKey].destacados[puntoDestacado] || 0) + 1; if (calificacionDesc === 'Negativa' || calificacionDesc === 'Muy Negativa') { if (comentario) dailyDetails[diaSemana].sectores[sectorKey].comentarios.push(comentario); } switch (calificacionDesc) { case 'Muy Positiva': processedData.general.muy_positivas++; processedData.porDia[diaSemana].muy_positivas++; processedData.porHora[hora].muy_positivas++; processedData.porSector[sectorKey].muy_positivas++; dailyDetails[diaSemana].sectores[sectorKey].muy_positivas++; dailyDetails[diaSemana].valoracionesPorHora[hora].muy_positivas++; dailyDetails[diaSemana].valoracionesPorHora[hora].sectoresPositivos[sectorKey] = (dailyDetails[diaSemana].valoracionesPorHora[hora].sectoresPositivos[sectorKey] || 0) + 1; if (comentario) processedData.nubes.positiva.push(...getWordsFromString(comentario)); break; case 'Positiva': processedData.general.positivas++; processedData.porDia[diaSemana].positivas++; processedData.porHora[hora].positivas++; processedData.porSector[sectorKey].positivas++; dailyDetails[diaSemana].sectores[sectorKey].positivas++; if (comentario) processedData.nubes.positiva.push(...getWordsFromString(comentario)); break; case 'Negativa': processedData.general.negativas++; processedData.porDia[diaSemana].negativas++; processedData.porHora[hora].negativas++; processedData.porSector[sectorKey].negativas++; dailyDetails[diaSemana].sectores[sectorKey].negativas++; if (comentario) processedData.nubes.negativa.push(...getWordsFromString(comentario)); break; case 'Muy Negativa': processedData.general.muy_negativas++; processedData.porDia[diaSemana].muy_negativas++; processedData.porHora[hora].muy_negativas++; processedData.porSector[sectorKey].muy_negativas++; dailyDetails[diaSemana].sectores[sectorKey].muy_negativas++; if (comentario) processedData.nubes.negativa.push(...getWordsFromString(comentario)); break; } } catch (e) { console.warn(`Se ignoró la fila ${rowNumber} por un error de formato.`); } });
        ...
        for (const dia of Object.keys(dailyDetails)) { const detallesSectoresDia = dailyDetails[dia].sectores; const sectoresCalculados = []; for (const nombreSector in detallesSectoresDia) { const statsSector = detallesSectoresDia[nombreSector]; if (statsSector.total > 0) { statsSector.satisfaccion = calculateSatisfaction(statsSector); sectoresCalculados.push({ nombre: nombreSector, stats: statsSector }); } } if (processedData.porDia[dia]) { processedData.porDia[dia].sectoresDelDia = sectoresCalculados; } }
        ...
        for (const dia in processedData.porDia) { let sectorMasCritico = { nombre: 'N/A', satisfaccion: 101, criticos: 'N/A', total: 0, comentarios: [] }; if (processedData.porDia[dia].sectoresDelDia) { processedData.porDia[dia].sectoresDelDia.forEach(({ nombre, stats }) => { if (stats.total < 3) return; if (stats.satisfaccion < sectorMasCritico.satisfaccion) { sectorMasCritico = { nombre: nombre, satisfaccion: stats.satisfaccion, criticos: getTopItems(stats.criticos, 3) || 'comentarios generales', total: stats.total, comentarios: stats.comentarios }; } }); } const picoPositivo = dailyDetails[dia].valoracionesPorHora.reduce((p, c, i) => c.muy_positivas > p.count ? { hora: i, count: c.muy_positivas } : p, { hora: -1, count: -1 }); const conclusionIA = analizarComentarioMasCritico(sectorMasCritico.comentarios, sectorMasCritico.criticos); processedData.porDia[dia].analisis = { picoPositivo: { hora: picoPositivo.hora, count: picoPositivo.count, sectores: picoPositivo.hora !== -1 ? getTopItems(dailyDetails[dia].valoracionesPorHora[picoPositivo.hora].sectoresPositivos) : 'N/A' }, sectorCritico: sectorMasCritico, conclusionIA: conclusionIA }; }
