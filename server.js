...
function getWordsFromString(text) {
    ...
    return text.toLowerCase().match(/\b(\w+)\b/g)?.filter(word => !STOPWORDS.includes(word) && word.length > 3) || [];
}
...
// --- RUTA PRINCIPAL PARA PROCESAR EL ARCHIVO ---
app.post('/procesar', upload.single('archivoExcel'), async (req, res) => {
    try {
            ...
            try {
                ...
                if (!processedData.porDia[diaSemana]) {
                    ...
                    dailyDetails[diaSemana] = {
                        valoracionesPorHora: Array.from({ length: 24 }, () => ({
                            muy_positivas: 0,
                            muy_negativas: 0,
                            sectoresPositivos: {},
                            sectoresNegativos: {}
        ...
        const getTopItems = (obj, count = 3) =>
            Object.entries(obj).sort(([, a], [, b]) => b - a).slice(0, count).map(([name]) => name).join(', ');
        ...
        processedData.nubes = {
            positiva: getWordsFromString(processedData.comentarios.positivos.join(' ')),
            negativa: getWordsFromString(processedData.comentarios.negativos.join(' '))
        };
